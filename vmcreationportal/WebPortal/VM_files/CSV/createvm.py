...
def get_ovf_descriptor(ovf_path):
    ...
    if path.exists(ovf_path):
		...
		error = "Could not read ovf file"
                send_error_mail_fn(error)
                raise SystemError("Could not read ovf file")
                exit(1)
...
def get_obj_in_list(obj_name, obj_list):
    ...
    error = "Unable to find object by the name of %s in list:\n%s" %(o.name, map(lambda o: o.name, obj_list))
    send_error_mail_fn(error)
    raise SystemError("Error in finding object")
    exit(1)
